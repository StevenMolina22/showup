...
export default function EditEventPage({ params }: EditEventPageProps) {
  ...
  const loadEvent = () => {
    ...
    setTimeout(() => {
      const event = getNativeEventById(eventId);
      if (event) {
        // Convert event data to form format
        const startAtLocal = new Date(event.startAt).toISOString().slice(0, 16);
        const endAtLocal = new Date(event.endAt).toISOString().slice(0, 16);
        ...
        // Event not found, redirect to manage page
        router.push("/manage");
      }
      setIsLoading(false);
    }, 300);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update the event
      const updates: Partial<Omit<NativeEvent, "id" | "createdAt">> = {
      ...
      const updatedEvent = updateNativeEvent(eventId, updates);

      if (updatedEvent) {
        // Redirect to the event detail page
        router.push(`/manage/events/${eventId}`);
      } else {
        alert("Failed to update event. Event may have been deleted.");
        router.push("/manage");
      }
    } catch (error) {
